...
def recursive_splitting(node: Node, features: pd.DataFrame, target: pd.Series) -> None:
    """Recursive splitting"""
    # check whether the current node is a leaf
    if (
            features.shape[0] <= 1  # Only one sample remains
            or gini_impurity(target.tolist()) == 0  # All targets are the same (pure node)
            or (features.nunique(axis=0) == 1).all()  # All features have the same value across all rows
    ):
        node.set_term(target.mode().iloc[0])  # The node is a leaf
        return

    # If the node is not a leaf, call the splitting function
    best_gini, best_feature, best_value, best_left_indexes, best_right_indexes = split_node(features, target)
    ...
    recursive_splitting(node.left, left_features, left_target)
    recursive_splitting(node.right, right_features, right_target)


def stage_3():
    path = input()  # r'test/data_stage3.csv'
    df = pd.read_csv(path, index_col=0)
    features = df.drop(columns=['Survived'])
    target = df.Survived
    root = Node()

    recursive_splitting(root, features, target)
